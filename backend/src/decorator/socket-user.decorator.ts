  ...
  (data: unknown, ctx: ExecutionContext) => {
    ...
    if (jwt === 'undefined' || jwt === null) {
      throw new UnauthorizedException(
        'Authentification failed, please log in again.',
      );
    }

    const jwtService = new JwtService({});
    const user: { sub: string; email: string; iat: string; exp: string } | any =
    ...
    if (user) {
      ...
      if (!connectedUser) {
        throw new NotFoundException(
          'Account has been deleted, new registration needed.',
        );
      } else {
